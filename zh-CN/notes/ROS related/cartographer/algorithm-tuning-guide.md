...
Cartographer 是一个复杂的系统，调优它需要对其内部工作原理有良好的理解。本页试图对 Cartographer 使用的不同子系统及其配置值给出直观的概述。如果您想了解更多关于 Cartographer 的信息，建议参考 Cartographer 的论文。该论文仅描述了 2D SLAM，但严格定义了这里描述的大部分概念。这些概念通常也适用于 3D 场景。
...
第一个子系统是**局部 SLAM**（有时也称为**前端**或局部轨迹构建器）。其工作是构建一系列**子图（submaps）**。每个子图都应该是局部一致的，但我们也承认局部 SLAM 随时间漂移。
...
另一个子系统是**全局 SLAM**（有时也称为**后端**）。它在后台线程中运行，其主要任务是找到**回环约束（loop closure constraints）**。它通过将在**节点(nodes)** 中收集的**扫描(scans)** 与子地图进行扫描匹配来实现。它还结合其他传感器数据，以获得更高层次的视图并确定最一致的全局解决方案。在 3D 中，它还尝试确定重力方向。
...
从更高的抽象层次来看，**局部 SLAM 的工作是生成良好的子地图**，**全局 SLAM 的工作是以最一致的方式将它们连接在一起**。

## 输入数据处理

### 测距传感器数据过滤

测距传感器（例如：激光雷达）可以提供多个方向的深度信息。然而，并非所有测量值都与 SLAM 相关。如果传感器部分被灰尘覆盖或指向机器人的某个特定部位，则某些测量距离可以被视为 SLAM 的噪声。另一方面，一些远距离的测量值也可能来自不必要的来源（反射、传感器噪声），对 SLAM 也不相关。
...
距离是在机器人实际移动时在一段时间内测量的。然而，传感器以大型 ROS 消息的形式"批量"传递距离数据。Cartographer 可以独立分析每条消息的时间戳，以考虑机器人运动引起的变形。Cartographer 越频繁地获得测量值，它就越能更好地展开测量值以组装一个本可以瞬间捕获的单一连贯扫描。

因此，强烈建议每次扫描（可以与另一次扫描匹配的一组距离数据）提供尽可能多的距离数据（ROS 消息）。
...
`CeresScanMatcher` 将初始猜测作为先验信息，并找到扫描与子地图匹配的最佳位置。它通过对子地图进行插值和对扫描进行亚像素对齐来实现。这种方法速度很快，但无法修复远大于子图分辨率的误差。

如果您的传感器设置和时序合理，仅使用 `CeresScanMatcher` 通常是最佳选择。

#### 2. 实时相关扫描匹配器（RealTimeCorrelativeScanMatcher）

如果您没有其他传感器或不信任它们，可以启用 `RealTimeCorrelativeScanMatcher`。它使用类似于闭环分析中扫描与子图匹配的方法（稍后会详细介绍），但是它匹配的是当前子地图。然后将最佳匹配结果用作 `CeresScanMatcher` 的先验信息。
...
局部 SLAM 会随时间发生漂移，全局 SLAM 用于修复此漂移。子图必须足够小，以使其内部的漂移低于分辨率，从而保证局部精度。另一方面，子地图又必须足够大，以确保其彼此独立，从而使闭环检测机制正常工作。
...
> **注意：** Cartographer ROS 提供了一个 RViz 插件来可视化子地图。您可以从编号中选择要查看的子图。在 3D 模式下，RViz 仅显示 3D 混合概率栅格的 2D 投影（灰度）。RViz 左侧面板中提供了在低分辨率和高分辨率混合栅格可视化之间切换的选项。

**TODO**: *记录 TSDF 配置*

## 全局 SLAM

当局部 SLAM 生成其子图序列时，全局优化任务（通常称为"*优化问题*"或"*稀疏位姿调整*"）在后台线程中运行。其作用是重新排列各个子地图，使它们形成一个连贯的全局地图。

例如，此优化负责改变当前构建的轨迹，以便根据循环闭合正确对齐子图。
...
全局 SLAM 是一种"*GraphSLAM*"，它本质上是一种位姿图优化，通过在**节点**和子地图之间构建**约束**，然后优化生成的约束图来工作。
