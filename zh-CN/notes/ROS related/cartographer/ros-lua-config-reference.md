...
话题名称在 Cartographer 的 ROS 集成中作为**基础名称**（参见 [ROS Names](http://wiki.ros.org/Names)）给出。这意味着由 Cartographer 节点的用户来决定重新映射话题，或将它们放入命名空间。
...
### map_frame
用于发布子图的 ROS 坐标系 ID，位姿的父坐标系，通常是 `"map"`。

### tracking_frame
SLAM 算法跟踪的坐标系的 ROS 坐标系 ID。如果使用 IMU，它应该位于 IMU 的位置，尽管可能是旋转的。常见的选择是 `"imu_link"`。
