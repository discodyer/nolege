...
## 摘要
便携式激光测距仪（以下简称激光雷达/LIDAR）与同步定位与地图构建（SLAM）技术是获取竣工平面图的高效方法。实时生成并可视化平面图有助于操作人员评估采集数据的质量和覆盖范围。构建便携式采集平台需在有限的计算资源下运行。本文提出一种应用于背包式测绘平台的方法，该方法能够以5cm的分辨率实现实时测绘与闭环检测。为达成实时闭环检测，我们采用分支定界法计算扫描与子图的匹配结果作为约束条件。实验结果及与其他主流方法的对比表明，该方法在精度方面与现有成熟技术具有竞争力。
...
SLAM技术在该领域的应用并非全新理念，也不是本文的研究重点。本文的核心贡献是提出一种新型方法，用于降低基于激光测距数据计算闭环约束的计算开销。该技术使我们能够对数万平方米的大型楼层进行测绘，同时为操作人员实时提供完全优化的结果。
...
像素级精确扫描匹配方法（如文献[^1]）进一步降低了局部误差累积。尽管计算开销更高，但该方法同样适用于闭环检测。部分方法通过匹配激光扫描提取的特征来降低计算成本[^4]。其他闭环检测方法包括基于直方图的匹配[^6]、扫描数据中的特征检测以及机器学习方法[^7]。
...
1. 当子图构建完成（即不再插入新的扫描数据）后，该子图将参与闭环检测（loop closure）的扫描匹配过程；
2. 所有已完成的子图和扫描数据都会被自动纳入闭环检测考虑范围，若根据当前位姿估计判断二者距离足够近，扫描匹配器将尝试在子图中寻找对应的扫描数据；
3. 如果在当前估计位姿周围的搜索窗口内找到足够优的匹配结果，则将其作为闭环约束（loop closing constraint）添加到优化问题中；
4. 通过每几秒执行一次优化，操作人员在重访某个位置时能够实时实现闭环检测。

为满足软实时（soft real-time）要求，闭环扫描匹配的执行速度必须快于新扫描数据的生成速度，否则会出现明显的滞后。我们通过采用分支定界法（Branch-and-Bound Approach）以及为每个已完成子图预计算多个栅格（precomputed grids）的方式，满足了这一要求。
...
## 五、闭环检测
由于扫描数据仅与包含近期少数扫描帧的子图进行匹配，上述方法会缓慢累积误差。对于几十帧连续扫描数据，累积误差较小；对于更大空间，则通过构建多个小子图来处理。我们的方法遵循稀疏位姿调整（Sparse Pose Adjustment）[^2]的思路，优化所有扫描数据和子图的位姿——扫描数据插入时的相对位姿被存储在内存中，用于闭环优化；当子图不再更新后，所有扫描与子图的组合都将被纳入闭环检测考虑范围，后台运行扫描匹配器，若找到优匹配结果，则将对应的相对位姿添加到优化问题中。

### （一）优化问题
与扫描匹配类似，闭环优化也被表述为非线性最小二乘问题，便于添加残差项以融合额外数据。每几秒使用Ceres求解器[^14]求解以下优化问题：
...
- CPU总耗时为1018s，内存占用最高2.2GB，最多使用4个后台线程进行闭环扫描匹配；
- 实际运行时间为360s，实时性能达到5.3倍；
- 闭环优化的生成图包含11456个节点和35300条边；
...
## 七、结论
本文提出并通过实验验证了一种二维SLAM系统，该系统结合扫描与子图匹配、闭环检测和图优化技术：
1. 通过局部基于栅格的SLAM方法构建独立的子图轨迹；
2. 后台利用像素级精确扫描匹配将所有扫描数据与邻近子图进行匹配，生成闭环约束；
3. 定期对包含子图和扫描位姿的约束图进行后台优化；
