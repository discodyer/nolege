...
## 为什么 3D 数据包中的激光雷达数据频率高于 VLP-16 官方标称的最高 20Hz 转速？ {#vlp16-hz-issue}
...
## 为什么 3D SLAM 需要 IMU 数据而 2D 不需要？ {#imu-requirement}

在 2D 中，Cartographer 支持运行相关扫描匹配器，该匹配器通常用于查找回环约束，也可用于局部 SLAM。虽然计算成本较高，但通常可以使里程计或 IMU 数据的整合变得不必要。2D 还有假设平面世界的好处，即"向上"方向是隐式定义的。
...
## 如何在不支持 rviz 的情况下构建 cartographer_ros？ {#no-rviz-build}

最简单的解决方案是在 `cartographer_rviz` 包目录中创建一个名为 [`CATKIN_IGNORE`](http://wiki.ros.org/catkin/workspaces) 的空文件。

## 如何修复 "You called InitGoogleLogging() twice!" 错误？ {#googlelogging-error}
